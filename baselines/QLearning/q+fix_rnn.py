...
class W_Module(nn.Module):
    n_agents: int
    hypernet_embed: int = 512 
    multi_output: bool=False     # Determines the output dimensions

    @nn.compact
    def __call__(self, states, actions):
        output_dim = self.n_agents if self.multi_output else 1
        data = jnp.concatenate([states, actions], axis=-1)  # Input to the network

        # Define the network
        x = nn.Dense(features=self.hypernet_embed)(data)    # First linear layer
        x = nn.relu(x)                                      # ReLU activation
        x = nn.Dense(features=output_dim)(x)                # Output layer

        return x


class B_Module(nn.Module):
    hypernet_embed: int=512     # Hidden layer dimension

    @nn.compact
    def __call__(self, states):
        x = nn.Dense(features=self.hypernet_embed)(states)  # First linear layer
        x = nn.relu(x)                                      # ReLU activation
        x = nn.Dense(features=1)(x)                         # Second linear layer
        return x  
...
class MixingNetwork(nn.Module):
    ...
    @nn.compact
    def __call__(self, q_vals, chosen_action_q_vals, states, actions):    

        n_agents, time_steps, batch_size, num_actions = q_vals.shape    # (3, 128, 32, 10)

        w = W_Module(n_agents, self.hypernet_embed, self.multi_output,)(states, actions)
        w = w.reshape(time_steps, batch_size, self.n_agents if self.multi_output else 1, 1)     #(128, 32, 1, 1)
        

        b = B_Module(self.hypernet_embed,)(states)
        b = b.reshape(time_steps, batch_size, 1, 1)
        b = b.squeeze(-1)       # (128, 32, 1)
        

        V_vals = q_vals.max(axis=-1)    # (3, 128, 32)
        V_vals = jnp.transpose(V_vals, (1,2,0))     # (128, 32, 3)

        chosen_action_q_vals = jnp.transpose(chosen_action_q_vals, (1,2,3,0))       # (128, 32, 1, 3)
        sum_chosen_action_q_vals = jnp.sum(chosen_action_q_vals, axis=-1)  # (128, 32, 1)

        sum_V_vals = jnp.sum(V_vals, axis=-1)   # (128, 32)
        sum_V_vals = jnp.expand_dims(sum_V_vals, -1)  # (128, 32, 1)

        
        adv_chosen_actions = sum_chosen_action_q_vals - sum_V_vals    # (128, 32, 1)
        adv_chosen_actions = jnp.expand_dims(adv_chosen_actions, -1)    # (128, 32, 1, 1)
        adv_chosen_actions = jax.lax.stop_gradient(adv_chosen_actions)  # implement detach

        w = jnp.abs(w + 1) - 1 + 1e-10      # implement weight constraint w > -1
        weighted_sum_of_adv = jnp.matmul(adv_chosen_actions, w).squeeze(-1)

        q_tot = weighted_sum_of_adv + b + sum_chosen_action_q_vals
...
def make_train(config, env):
    ...
    def train(rng):
        ...
        mixer = MixingNetwork(hypernet_embed=config["HIDDEN_SIZE"], multi_output=config["MULTI_OUTPUT"],)
        ...
        def create_agent(rng):
            ...
            # init mixer
            action_size = wrapped_env.max_action_space
            rng, _rng = jax.random.split(rng)
            init_x = jnp.zeros((len(env.agents), 1, 1, action_size)) # q vals: agents, time, batch
            init_chosen_x = jnp.zeros((len(env.agents), 1, 1, 1))
            
            # TODO: Find a way to not hard-code 96 as obs_size
            init_state = jnp.zeros((1, 1, 1, 96)) # (time_step, batch_size, obs_size)
            init_action = jnp.zeros((1, 1, 1, action_size*len(env.agents)))
            mixer_params = mixer.init(_rng, init_x, init_chosen_x, init_state, init_action)
        ...
        _env_obs, _env_state = wrapped_env.batch_reset(rng)
        ...
        def _update_step(runner_state, unused):
            ...
            def _learn_phase(carry, _):
                ...
                _actions = batchify(minibatch.actions)  #  (3, 128, 32) (num_agents, time_steps, batch_size)
                ...
                def _loss_fn(params):
                    ...
                    chosen_action_q_vals = jnp.take_along_axis(q_vals,
                        _actions[..., np.newaxis], axis=-1,)#.squeeze(-1)  # (num_agents, timesteps, batch_size,)
                    ...
                    q_next = jnp.take_along_axis(q_next_target,
                        jnp.argmax(valid_q_vals, axis=-1)[..., np.newaxis], axis=-1,)#.squeeze(-1)  # (num_agents, timesteps, batch_size,)


                    max_actions = jnp.argmax(valid_q_vals, axis=-1)
                    one_hot_max_actions = jax.nn.one_hot(max_actions, num_classes=wrapped_env.max_action_space)     #[3,128,32,10]
                    joint_max_actions = jnp.concatenate(one_hot_max_actions, axis=-1)  # Shape: (128, 32, 30)

                    vdn_next = mixer.apply(train_state.target_network_params['mixer'], q_next_target, q_next, minibatch.obs["__all__"], joint_max_actions)
                    vdn_next = vdn_next.squeeze(-1)
                    ...
                    vdn_target = (
                        ...
                        * config["GAMMA"]
                        * vdn_next[1:]  # sum over agents
                    )

                    vdn_target = jnp.expand_dims(vdn_target, -1)


                    one_hot_actions = {}
                    for agent, action_values in minibatch.actions.items():
                        one_hot_actions[agent] = jax.nn.one_hot(action_values,  wrapped_env.max_action_space)  # Shape: (128, 32, 10)

                    joint_action = jnp.concatenate([one_hot_actions[agent] for agent in minibatch.actions], axis=-1)  # Shape: (128, 32, 30)


                    chosen_action_q_vals = mixer.apply(params['mixer'], q_vals, chosen_action_q_vals, minibatch.obs['__all__'], joint_action)[:-1]
